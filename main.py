...
class Config:
    def __init__(self):
        ...
        self.claude_model: str = "claude-3-5-sonnet-20240620"
...
# In the main script, update the config loading part:
config = Config()
...
async def execute_goals(goals: List[str]):
    global automode
    for i, goal in enumerate(goals, 1):
        print_colored(f"\nExecuting Goal {i}: {goal}", TOOL_COLOR)
        response, _ = await chat_with_claude(f"Continue working on goal: {goal}")
        if CONTINUATION_EXIT_PHRASE in response:
            automode = False
            print_colored("Exiting automode.", TOOL_COLOR)
            break

async def chat_with_claude(user_input: str, image_path: Optional[str] = None, current_iteration: Optional[int] = None, max_iterations: Optional[int] = None) -> Tuple[str, bool]:
